...
const isHiddenCloud = /hidenplay\.net$/i.test(window.location.hostname);
const isRenderFrontend = /vivachilerpcm-mfbn\.onrender\.com$/i.test(window.location.hostname);

// Para HiddenCloud y Render, el mismo subdominio sirve ambos (frontend y backend)
const defaultAppBase = (isHiddenCloud || isRenderFrontend) ? currentOrigin : currentOrigin;
const defaultBackendBase = (isHiddenCloud || isRenderFrontend) ? currentOrigin : "";
